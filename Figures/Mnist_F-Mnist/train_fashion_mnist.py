...
def get_args():
    ...
    parser.add_argument('--attack', default='fgsm', type=str, choices=['none', 'pgd', 'fgsm', 'fgsm_fast'])
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for i, (X, y) in enumerate(train_loader):
            ...
            if i == 95:
                plt.subplot(131)
                plt.title(r'$\epsilon = 0$', fontsize=18)
                ...
                plt.subplot(132)
                plt.title(r'$\epsilon = 0.2$', fontsize=18)

                delta = torch.zeros_like(X).uniform_(-0.2, 0.2)
                delta.data = torch.max(torch.min(1-X, delta.data), 0-X)
                for _ in range(args.attack_iters):
                    delta.requires_grad = True
                    output = model(X + delta)
                    ...
                    grad = delta.grad.detach()
                    I = output.max(1)[1] == y
                    delta.data[I] = torch.clamp(delta + 0.25 * torch.sign(grad), -0.2, 0.2)[I]
                    delta.data[I] = torch.max(torch.min(1-X, delta.data), 0-X)[I]
                delta = delta.detach()
                plt.imshow(torch.clamp(X + delta, 0, 1).cpu()[0][0], cmap='gray')


                plt.subplot(133)
                plt.title(r'$\epsilon = 0.3$', fontsize=18)
                ...
                for _ in range(args.attack_iters):
                    ...
                    grad = delta.grad.detach()
                    I = output.max(1)[1] == y
                    delta.data[I] = torch.clamp(delta + args.alpha * torch.sign(grad), -0.3, 0.3)[I]
                    delta.data[I] = torch.max(torch.min(1-X, delta.data), 0-X)[I]
                delta = delta.detach()
                plt.imshow(torch.clamp(X + delta, 0, 1).cpu()[0][0], cmap='gray')
                ...
                plt.savefig("fig_F-MNIST_epsilons.pdf")
                plt.show()   
